/*
  From Elliot Williams https://github.com/hexagon5un/AVR-Programming/
...
/* Print a signed 16-bit integer as decimal text.  Handles negative values. */
void printInt16(int16_t value) {
  int32_t tmp = value; /* promote to signed 32-bit to handle -32768 */
  if (tmp < 0) {
    transmitByte('-');
    tmp = -tmp;
  }
  /* tmp now non-negative */
  if (tmp == 0) {
    transmitByte('0');
    return;
  }
  /* collect digits in buffer (reverse) */
  char buf[12];
  int i = 0;
  while (tmp > 0 && i < (int)sizeof(buf)) {
    buf[i++] = '0' + (tmp % 10);
    tmp /= 10;
  }
  while (i--) {
